...
impl Node for () {
    fn init_child(&mut self, _f: Box<dyn FnOnce() -> AsyncNode>, _add_widget: bool) -> (AsyncNode, bool) {
        panic!("{}", PANIC_MSG);
    }

    fn init_sibling(&mut self, _f: Box<dyn FnOnce() -> AsyncNode>, _add_widget: bool) -> (AsyncNode, bool) {
        panic!("{}", PANIC_MSG);
    }
